...
`SKILL.md` 是 Agent 真正读取的文件。它包含管道架构、每步的 bash/Python 命令、输入输出契约、以及每种失败场景的处理方案。Agent 加载后就知道如何编排 7 个步骤。

`scripts/parse_bookmark_hierarchy.py` 是唯一可独立运行的脚本。它将书葵网返回的扁平书签文本解析为多级嵌套树。运行 `python3 scripts/parse_bookmark_hierarchy.py`（无参数）会执行 4 组内置测试并输出解析结果。

`references/evaluation-cases.md` 包含 7 个评测用例和一条「零基础设施」可跑路径。如果你第一次部署，从这里开始——它假设你没有 EbookDatabase、没有 stacks、没有 Z-File，只验证 Anna's Archive + OCR 的最小闭环。
