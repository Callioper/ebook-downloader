...
- 用户说「配置 ebook-downloader」「设置 ebook 管道」「初始化 ebook」或「setup ebook downloader」
- Agent 应加载 `references/setup-guide.md`，按 7 项逐项引导用户选配功能
- 选配完成后输出环境变量模板供用户保存
...
完整的评测用例和最小可跑路径见 `references/evaluation-cases.md`。包含：

- **零基础设施可跑路径** — 不需要 EbookDatabase、stacks、Z-File，只验证 Anna's Archive 搜索 + OCR
- **7 个评测用例** — 覆盖书名/ISBN/SS码三种入口、书签三层降级、OCR 跳过、OCR 乱码防御
- **管道完整性自检清单** — 5 项检查确认管道就绪

首次部署建议从 evaluation-cases.md 的最小可跑路径开始。
